...
export const Header: React.FC<HeaderProps> = ({ currentPage, onNavigate }) => {
  ...
  const navLinks = [
    ...
    { name: 'Buy', id: 'buy', action: () => onNavigate('home') },
  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled || currentPage !== 'home' ? 'bg-white shadow-md py-3' : 'bg-transparent py-5'
      }`}
    >
          ...
          {navLinks.map((link) => (
            <button 
              ...
              className={`text-sm font-medium hover:text-secondary transition-colors ${
                currentPage === link.id ? 'text-secondary font-bold' : 
        ...
        <div className="absolute top-full left-0 w-full bg-white shadow-xl border-t border-slate-100 lg:hidden flex flex-col p-6 animate-in slide-in-from-top-5">
          {navLinks.map((link) => (
            <button 
              ...
              className={`py-3 text-left font-medium border-b border-slate-100 last:border-0 hover:text-secondary ${
                currentPage === link.id ? 'text-secondary' : 'text-slate-700'
              }`}
            >
              {link.name}
            </button>
